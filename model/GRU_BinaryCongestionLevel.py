...
# 4. 二值化標籤：1~2 → 0 (無壅塞)，3~5 → 1 (有壅塞)
y = np.array([1 if t >= 3 else 0 for t in targets], dtype=int)
...
# 6. 切分訓練/驗證/測試集 (stratify by section_ids)
...
pd.DataFrame(records).to_csv(f'GRU_binary_0501_0521_loss_v{model_version}.csv', index=False)
pd.DataFrame(metrics_records).to_csv(f'GRU_binary_0501_0521_metrics_v{model_version}.csv', index=False)
...
df_pred.to_csv(f'GRU_binary_0501_0521_predictions_v{model_version}.csv', index=False)
